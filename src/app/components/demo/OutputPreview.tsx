...
type ViewMode = 'preview' | 'markdown' | 'raw';
...
export function OutputPreview({ content }: OutputPreviewProps) {
  ...
  const handleCopy = async () => {
    const markdownContent = content.article?.body || content.final_blog;
    const textToCopy = viewMode === 'markdown' ? markdownContent :
      ...
      case 'preview':
        return (
          <article className="prose prose-lg max-w-none">
            <h1>{content.article?.title || 'Untitled Article'}</h1>
            {content.article?.description && (
              <p className="lead">{content.article.description}</p>
            )}
            <pre className="whitespace-pre-wrap text-sm text-gray-800 bg-transparent border-0 p-0 font-inherit">
              <code>{content.article?.body || content.final_blog}</code>
            </pre>
          </article>
        );
  ...
  const viewModes = [
    ...
    { key: 'markdown' as ViewMode, label: 'Markdown', icon: Code },
